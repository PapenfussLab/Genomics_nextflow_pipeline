...
# make output dir
mkdir ${tumourid}_octopus

# merge vcf files
IFS=',' read -r -a vcf_array <<< $vcf_list
gatk MergeVcfs ${vcf_array[@]/#/-I } -O ${tumourid}_octopus/${tumourid}.octopus.vcf
...
perl ${vcf2maf}/vcf2maf.pl \
    ...
    --vep-path ${vep_path} \
    --vep-data ${refDir}/vep \
    --input-vcf ${tumourid}_octopus/${tumourid}.octopus.vcf \
    --output-maf ${tumourid}_octopus/${tumourid}.octopus.maf \
    --tmp-dir ${tumourid}_octopus \
    --tumor-id ${tumourid} \
    --ref-fasta  ${ref} \
    --ncbi-build ${genome} \
    --cache-version ${cache_version} \
    --retain-fmt AF,DP,AD,ADP
